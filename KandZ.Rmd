...
``` {r}
final_dataset2 %>%
  ggplot(aes(x = number_com, y = total_contributions, color = incumbent, size = primary_votes.x)) +
  geom_point() 
```

```{r}
final_dataset2 %>%
  ggplot(aes(x = number_com, y = total_contributions, color = party, size = primary_votes.x)) +
  geom_point()
```